...
| Domain-first ApprovalSheet rendering. The headline is the **full host** of the metadata `url` minus one leading `www.`, ASCII-only (punycode is shown literally as `xn--…`; raw Unicode, percent-escapes, IP literals, localhost and single-label hosts yield no domain), and **never collapsed** — a public-suffix collapse would launder `attacker.github.io` into `github.io` (likewise pages.dev, vercel.app, netlify.app, …). With no usable url the headline is the short client-pubkey fingerprint; the self-asserted `name` **never** occupies the headline slot. Name/icon sit below, small, as 'calls itself “…”' (or 'icon' for an icon-only caller — marked too) followed by a fixed `· unverified` sibling that a long name can truncate *against* but never push off screen. Same rules and strings on the onboarding banner | `ApprovalSheet.swift`, `OnboardingView.swift`, `CallerIdentity.swift` | S |
| `callback=` support: display target in ApprovalSheet (same domain-first rendering as the caller); open only after a foreground approval; an https callback must have **exactly the same host** as the metadata `url` (compare the two `CallerIdentity.domain(fromURL:)` results — no registrable-domain collapse, so `attacker.github.io` can never call back as `github.io`; a mismatch drops the callback: not shown, not opened); custom-scheme callbacks are opened as given; never on denial; never from lock-screen signing. Note for web callers: an https callback opens a *new* Safari tab rather than the tab holding the pending pairing, so for web callers prefer a "Return to *host*" hint over auto-open — auto-return is for native callers (custom scheme or Universal Link) | `NostrConnectParser.swift`, `ApprovalSheet.swift` | M |
