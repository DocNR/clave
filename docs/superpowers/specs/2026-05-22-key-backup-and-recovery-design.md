...
- **NIP-49:** rust-nostr exposes `EncryptedSecretKey` (encrypt a `SecretKey` under a password → bech32 `ncryptsec`, and decrypt back). This avoids hand-implementing scrypt + **XChaCha20-Poly1305**, neither of which is available in Apple's CryptoKit (CryptoKit's `ChaChaPoly` is ChaCha20-Poly1305 with a 96-bit nonce — **not** the 192-bit XChaCha20 variant NIP-49 requires, and there is no scrypt primitive). Hand-rolling NIP-49 in the `Shared/LightCrypto` stack is therefore explicitly **not** preferred.
- **NIP-06:** rust-nostr exposes mnemonic generation and `Keys` derivation from a mnemonic (+ optional passphrase) at the NIP-06 path.

> **Verification task before planning:** confirm the 0.44.2 Swift bindings surface `EncryptedSecretKey` and the mnemonic-based `Keys` initializer under the exact symbol names. If a symbol is missing, the fallback is to bump `nostr-sdk-swift` or vendor a minimal scrypt + XChaCha20-Poly1305 + BIP-39/BIP-32 implementation — a materially larger task that the plan must size separately.
...
1. **rust-nostr binding coverage.** Confirm `EncryptedSecretKey` and mnemonic-`Keys` exist in 0.44.2 Swift bindings (see Crypto section). This gates whether Tier 1 is "wire up SDK calls" or "also implement scrypt + XChaCha20 + BIP-39/32." Resolve before plan sizing.
